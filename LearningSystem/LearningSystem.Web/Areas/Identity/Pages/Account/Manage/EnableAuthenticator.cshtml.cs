namespace LearningSystem.Web.Areas.Identity.Pages.Account.Manage
{
    ...
    public class EnableAuthenticatorModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var verificationCode = this.Input
                .Code
                .Replace(" ", string.Empty)
                .Replace("-", string.Empty);

            var is2faTokenValid = await this.userManager
                .VerifyTwoFactorTokenAsync(user, this.userManager.Options.Tokens.AuthenticatorTokenProvider, verificationCode);
            ...
            var userId = await this.userManager
                .GetUserIdAsync(user);

            this.logger
                .LogInformation("User with ID '{UserId}' has enabled 2FA with an authenticator app.", userId);
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                ...
                this.urlEncoder.Encode("LearningSystem.Web"),
